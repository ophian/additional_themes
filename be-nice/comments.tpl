...
{foreach $comments AS $comment}
    ...
    <li><div id="serendipity_comment_{$comment.id}" class="serendipity_comment serendipity_comment_author_{$comment.author|makeFilename} {if $entry.author == $comment.author}serendipity_comment_author_self{/if}{cycle values="comment_oddbox, comment_evenbox"}">
        ...
        <p>
            ...
            <span class="comment_source_date"><em>{$comment.timestamp|formatTime:$CONST.DATE_FORMAT_ENTRY} {$CONST.AT} <a class="comment_source_trace" href="#c{$comment.id}" title="{$CONST.PERMALINK_COMMENT}">{$comment.timestamp|formatTime:'%H:%M'}</a></em> </span>
            {if $comment.url}
            ...
            {if $entry.is_entry_owner}
                (<a class="comment_source_ownerlink" href="{$comment.link_delete}" onclick="return confirm('{$CONST.COMMENT_DELETE_CONFIRM|sprintf:$comment.id:$comment.author}');">delete</a>){/if}</p></div><br />
...
{foreach $comments AS $comment}
    ...
    <a id="c{$comment.id}"></a>
        <div class="serendipity_comment_source">
            ...
            {else}
                {$comment.author|default:$CONST.ANONYMOUS}
            {/if}{if $comment.depth == 0}{$CONST.SAYS}{elseif $comment.depth == 1}{$CONST.REPLIED}{else}{$CONST.ADDS_THIS}{/if}<br />
