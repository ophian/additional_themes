...
{if NOT empty($entries)}{* catch a staticpage startpage which has no $entries array set *}
    ...
    <a href="{$entry.link_author}"><img src="templates/translucency/img/icons/user.gif">{$CONST.POSTED_BY}: {$entry.author}</a> {$CONST.IN} <img src="templates/translucency/img/icons/category.gif">:{foreach $entry.categories AS $category}<a href="{$category.category_link}"> {$category.category_name|escape}</a>{if NOT $entry_category@last}, {/if}{/foreach}<br>
