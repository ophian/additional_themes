...
{if NOT empty($entries)}{* catch a staticpage startpage which has no $entries array set *}
    ...
    <div class="serendipity_Entry_Date">
        ...
        <h2 class="serendipity_title"><a href="{$entry.link}" title="{$CONST.VIEW_FULL}">{$entry.title|default:$entry.id}</a></h2>
        ...
        {if NOT $is_preview}
            ...
            {$CONST.POSTED_BY} <a href="{$entry.link_author}">{$entry.author}</a> {$CONST.ON} {$entry.timestamp|formatTime:DATE_FORMAT_ENTRY} {$CONST.AT} {$entry.timestamp|formatTime:'%H:%M'}
            {if NOT empty($entry.categories)}
