...
{if NOT empty($entries)}{* catch a staticpage startpage which has no $entries array set *}
    {foreach $entries AS $dategroup}
        ...
        <div class="serendipity_entry serendipity_entry_author_{$entry.author|makeFilename}{if NOT empty($entry.is_entry_owner)} serendipity_entry_author_self{/if} post">
            ...
            {if NOT $is_preview}
                ...
                {if $is_single_entry}
                    ...
                    {if $entry.categories}
                       {$CONST.FILED_UNDER} {if NOT empty($entry.categories)}{foreach $entry.categories AS $category}<a href="{$category.category_link}">{$category.category_name|escape}</a>{if NOT $category@last}, {/if}{/foreach}{/if}.
                    {/if}<br>
                ...
                {else}
                    ...
                    {if $entry.has_comments}
                        ...
                        {/if}
                    {else}
                        {if $use_popups AND $entry.allow_comments}
                            | <a href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{$CONST.ADD_COMMENT}</a>
                        {/if}
                    {/if}
...
{if NOT $is_single_entry AND NOT $is_preview AND NOT $plugin_clean_page AND (NOT empty($footer_prev_page) OR NOT empty($footer_next_page))}
    <div class="serendipity_entries_footer pnavigation">
